...
class ConversationModelConfig(Base):
    __tablename__ = "conversation_model_configs"

    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    conversation_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("conversations.id"), index=True, unique=True, nullable=False
    )
    default_model: Mapped[str | None] = mapped_column(String(255), nullable=True)
    node_models_json: Mapped[str] = mapped_column(Text, nullable=False, default="{}")
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
    updated_at: Mapped[datetime] = mapped_column(
...
class MetricRequest(Base):
    ...
    retrieved_tools_json: Mapped[str] = mapped_column(Text, nullable=False, default="[]")
    retrieved_documents_json: Mapped[str] = mapped_column(Text, nullable=False, default="[]")
...
class MetricEvent(Base):
    ...
    request_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("metric_requests.id", ondelete="CASCADE"), index=True, nullable=False
    )
